...
1. **移动端优先：** 在构建桌面端网页前先构建移动端网页
2. **使用 `@media` 规则：** 通过**媒体查询（media queries）**为不同视口大小指定样式
3. **流式布局（液体布局）：** 不给容器设置固定单位（如 `px`、`em`（因为 `em` 只能根据字体大小变化，无法根据视口宽度变化） 等），使用 `%`、Flexbox、Grid 等方式布局
...
3. 给页面添加 `meta` 标签
   ...
   **⚠️ 注意：** 不推荐将视口宽度设置为 `320px`，以及使用 `user-scalable=no` 阻止用户双指缩放页面
