...
有时只**需要为一个模式找到那些在另一个模式之后或之前的匹配项**。

有一种特殊的语法，**称为 “前瞻断言”（lookahead）和 “后瞻断言”（lookbehind）**。

例如，从字符串 `1 turkey costs 30€` 中查找价格，即：一个数字，后跟 `€` 符号。
...
**⚠️ 注意：前瞻断言只是一个测试，括号 `(?=...)` 中的内容不包含在匹配结果 `30` 中**。
...
换句话说，**这样的模式意味着同时在寻找 `X` 后跟 `Y` 和 `Z`**。

**⚠️ 注意：这只有在模式 `Y` 和 `Z` 不是互斥的情况下才可行**，例如 `\d+(?=\s)(?=.*30)` 查找后跟着空格 `(?=\s)` 的 `\d+`，并且有 `30` 在它之后的某个地方 `(?=.*30)`：
...
上述代码在给出的字符串中，与数字 `1` 完全匹配。



**否定的前瞻断言**

假设想要一个数量，而不是来自于同一字符串的价格，那是一个数字 `\d+`，后面不是 `€`。

为此，可以使用否定的前瞻断言。

语法：`X(?!Y)`，意思是 **“搜索 `X`，但前提是后面没有 `Y`”**。
...
**⚠️ 注意：早期非 V8 引擎的浏览器不支持后瞻断言，例如 Safari、Internet Explorer，不过目前最新版本（除 Internet Explorer 外）的浏览器都已经支持**。

前瞻断言允许添加一个 **“后面要跟着什么”** 的条件判断。

后瞻断言也类似，**只不过它是在相反的方向上进行条件判断**，也就是说，它**只允许匹配前面有特定的字符串的模式**。
...
例如，把价格换成美元，美元符号通常在数字的前面，所以要查找 `$30` 需要使用 `(?<=\$)\d+`：
...
如果需要找到量词 —— 一个前面不带 `$` 的数字，可以使用否定的后瞻断言 `(?<!\$)\d+`：
...
例如，在模式 `\d+(?!€)` 中，`€` 符号就不会出现在匹配结果中，这是很自然的事情：寻找一个数字 `\d+`，而 `(?=€)` 只是一个测试，表示要匹配的数字后面应该紧跟着 `€` 字符。

但在某些情况下，**可能还需要捕获前瞻断言和后瞻断言所匹配的内容，或者部分内容，这也是可行的，只需要将该部分包装在额外的括号中**。

在下面的示例中，货币符号 `(€|kr)` 和金额一起被捕获了：

```js
const str = '1 turkey costs 30€';
const regexp = /\d+(?=(€|kr))/; // €|kr 两侧有额外的括号
...
有些正则表达式看起来简单，**但执行起来耗时却非常长，甚至会导致 JavaScript 引擎 “挂起”**。

大多数开发者迟早会遇到这样的情况，**典型的症状就是正则表达式有时可以正常工作，但对于某些字符串，它会消耗 100% 的 CPU 算力，出现 “挂起” 的现象**。

在这种情况下，Web 浏览器会建议终止脚本并重新加载页面，这显然不是愿意看到的情况。

**对于服务器端 JavaScript，这样的正则表达式可能会挂起服务器进程，甚至更糟**。



**举例**

假设有一个字符串，想检查其中是否包含一些后面跟着可选空格 `\s?` 的单词 `\w+`。

此时的正则表达式最显而易见的方式就是 `\w+\s?`，然后用 `*` 重复它。
...
上述代码似乎能正常工作，结果是正确的，**但在特定的字符串上，它会消耗很多时间，以至于让 CPU 会跑满 100% 负载，导致 JavaScript 引擎 “挂起”**。

例如运行下面这个例子，由于 JavaScript 会导致进程 “挂起”，所以可能什么结果都看不到，此时浏览器会停止对事件的响应，UI 也会停止工作，一段时间之后，浏览器会建议重新加载页面：

```js
const regexp = /^(\w+\s?)*$/;
const str = 'An input string that takes a long time or even makes this regexp hang!';
...
简化前面的例子：移除空格符 `\s?`，使其简化为 `^(\w+)*$`。

为了让问题更明显，再用 `\d` 替换掉 `\w`，生成的新正则表达式执行时仍会导致挂起，例如：
...
首先，这个正则表达式的 `(\d+)*` 部分有点奇怪，量词 `*` 看起来没有什么必要，如果要匹配一个数字，可以使用 `\d+`。

**实际上，正则表达式很死板**。

在 `123456789z` 这行中搜索 `^(\d+)*$` 时的执行过程如下：
...
1. 首先，正则表达式引擎尝试查找括号中的内容：**数字 `\d+`，加号 `+` 默认为贪婪模式，所以它消耗了所有数字**：
   ...
   消耗完搜索数字后，认为找到了 `\d+`（如 `123456789`）。

   然后它尝试应用星号量词，**但此时已经没有更多数字了，所以星号没有给出任何信息**。

   模式中接下来的 `$` 匹配字符串的结束，但是例子的文字中有 `z`，所以匹配失败：
...
有很多种方式可以将数字序列 `123456789` 拆分为多个数字，**准确地说，有 `(2^n)-1` 种，其中 `n` 是序列的长度**。
...
在前面第一个例子中，当使用 `^(\w+\s?)*$` 这种模式在字符串 `An input that hangs!` 中查找单词时，就会发生类似的问题。
...
从人的视角来看，很显然它没法匹配成功，因为示例中的字符串以叹号 `!` 结尾，**然而正则表达式期望的是一个单词 `\w` 末尾有或没有空格 `\s`，但引擎理解不了这种状况**。

它尝试了 `(\w+\s?)*` 的所有排列组合视图去囊括整个字符串，包括带空格 `(\w+\s)*` 的情形和不带空格 `(\w+)*` 的情形（因为空格 `\s?` 是可选的），由于各种排列组合的数量太多了，所以耗费了大量时间去查询。

**不幸的是无法使用惰性模式 `\w+?` 去替代 `\w+`，还是会挂起，排列组合的顺序会变化，但总数不变**。
...
1. **减少可能的组合数量**
   ...
   可以把第一个例子中的正则表达式重写为 `^(\w+\s)*\w*$` 以使空格变为非可选的，将查找任意数量的单词后跟空格（`(\w+\s)*`），然后跟着最后一个单词 `\w*`（可选）。

   这个正则表达式等同于之前那个（匹配内容相同），并且运行起来也没问题：

   ```js
   const regexp = /^(\w+\s)*\w*$/;
   const str = 'An input string that takes a long time or even makes this regex hang!';
   ...
   **问题之所以消失了，是因为空格是强制性的**。
   ...
   前面的正则表达式如果省略空格，就会变成 `(\w+)*`，导致单个单词中有很多 `\w+` 组合。

   所以 `input` 可以匹配 `\w+` 两次重复，如下所示：
...
2. **防止回溯**
   ...
   **有时候重写正则表达式会比较麻烦**，在前面的示例中，这很容易，但如何做到这一点并不总是很明显。

   **此外，重写的正则表达式通常更复杂，不利于维护**。

   幸运的是，还有另一种方式，可以禁止量词的回溯。

   **问题的根源在于正则表达式引擎尝试了许多对人类看来显然是错误的组合**。

   例如，正则表达式 `(\d+)*$` 中 `+` 对人类来说很明显不应该去回溯，就算用两个单独的 `\d+\d+` 去替换一个 `\d+`，也根本没变化：
   ...
   在原先的例子 `^(\w+\s?)*$` 中，可能希望在 `\w+` 中禁止回溯，即：`\w+` 应该匹配一个完整的单词，并且具有最大可能的长度，无需降低 `\w+` 重复次数或拆分为两个单词 `\w+\w+` 等等。
   
   为此，**现代正则表达式引擎支持占有型量词（Possessive Quantifiers），如果在常规量词之后添加 `+`，则常规量词就变成了占用型量词，也就是说，可以使用 `\d++` 替代 `\d+` 来阻止 `+` 回溯**。
   
   **占有型量词实际上比 “常规” 量词更简单，它们只是尽可能多地匹配，没有任何回溯，没有回溯的搜索过程更简单**。
   
   还有所谓的 “原子捕获组” —— 一种禁用括号内回溯的方法，**但坏消息是 JavaScript 并不支持它，不过可以通过 “前瞻变换” 来模拟它们**。
   
   
   
   **用前瞻视角解决问题**
   
   当希望量词，例如 `+` 不要回溯，因为回溯没有意义。
   
   **在不回溯的情况下尽可能多地重复 `\w` 的模式可以写为：`(?=(\w+))\1`**，除了这种方式也可以采用另一种模式来代替 `\w`。
   ...
   也就是说：先进行前瞻查找 —— 如果有符合 `\w+` 的单词，就可以将其匹配为 `\1`。
   
   之所以这样，是**因为前瞻断言查找到一个单词 `\w+`，将其作为一个整体，然后将其捕获为 `\1`**，所以最终实现了一种占有型加号 `+` 量词，**它只捕获整个单词 `\w+`，而不会只捕获一部分**。
   
   例如，在单词 `JavaScript` 中不仅可以匹配 `Java`，而且可以忽略 `Script`，以匹配模式的剩余部分。
   ...
   **所以当需要禁止 `+` 进行回溯，只需要把 `(?=(\w+))\1` 中的 `\w` 替换成更复杂的正则表达式就可以实现了**。
   
   例如使用前瞻断言重写第一个例子中的正则表达式来防止回溯：
   ...
   上述代码中使用的是 `\2` 而不是 `\1`，因为这里有额外的括号，**为了防止数字弄混，可以给括号命名**，例如 `(?<word>\w+)`：
...
**`y` 修饰符能够在源字符串中的指定位置进行搜索**。

**正则表达式的常见任务之一就是 “词法分析”**：例如得到了一个代码文本，需要找到它的结构元素，例如 HTML 有标签和特性（attribute），JavaScript 代码有函数、变量等。

编写词法分析器是一个特殊的领域，有自己的工具和算法，但有一个共同的任务：**在给定的位置读取一些内容**。

例如，有一个代码字符串 `let varName = "value"`，需要从中读取变量名，这个变量名从位置 `4` 开始。

可以使用正则表达式 `\w+` 来查找变量名，实际上，JavaScript 的变量名需要更复杂的正则表达式才能准确匹配。

- 调用 `str.match(/\w+/)` 将只会找到该行中的第一个单词（`let`），但不是这个
- 可以添加修饰符 `g`，但要调用 `str.match(/\w+/g)` 会查找文本中的所有单词，但只需要位置 `4` 的一个单词，同样也不能使用这种方法

**可以使用方法 `regexp.exec(str)` 在给定的位置准确搜索正则表达式**。

**对于没有修饰符 `g` 和 `y` 的 `regexp`，此方法仅查找第一个匹配项**，就像 `str.match(regexp)` 一样。

**但如果有修饰符 `g`，那么它就会从存储在 `regexp.lastIndex` 属性中的位置开始在字符串 `str` 中进行搜索，如果找到匹配项，则将在匹配后立即将 `regexp.lastIndex` 设置为索引**。

换句话说，`regexp.lastIndex` 作为搜索的起点，每个 `regexp.exec(str)` 调用都会将其重置为新值（“在最后一次匹配后”），当然，这只是在有 `g` 修饰符的情况下才会这样。

因此，**连续调用 `regexp.exec(str)` 会一个接着一个地返回匹配**。

以下是此类调用的示例：

```js
const str = 'let varName'; // 找出字符串中的所有单词
const regexp = /\w+/g;
...
`regexp.exec` 的这种方式**可以作为 `str.matchAll` 方法的替代，可以对匹配过程进行更多控制**。

回到前面说的任务，可以手动将 `lastIndex` 设置为 `4`，从给定的位置开始搜索。
...
const regexp = /\w+/g; // 没有修饰符 "g"，lastIndex 属性会被忽略
...
现在问题解决了，从位置 `regexp.lastIndex = 4` 开始搜索 `\w+`。

**⚠️ 注意：`regexp.exec` 调用从位置 `lastIndex` 开始搜索，然后继续搜索，如果 `lastIndex` 位置没有单词，但单词在这之后的某个位置，那么单词也会被找到：**
...
**这对于某些任务，包括词法分析，这是错误的，需要在文本的给定位置准确地找到匹配，而不是在它之后的某个位置，这就是修饰符 `y` 的用途**。
...
regexp.lastIndex = 3;
alert( regexp.exec(str) ); // null（位置 3 是一个空格，不是单词）

regexp.lastIndex = 4;
alert( regexp.exec(str) ); // varName（在位置 4 的单词）
```

正则表达式 `/\w+/y` 在位置 `3` 处不匹配（不同于修饰符 `g`），但在位置 `4` 处匹配。

这不仅是所需要的，**当使用修饰符 `y` 时，还有一个重要的性能提升**。
