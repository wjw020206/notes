...
这两种形状分别由不同级别的 CSS Shapes 规范定义，其中 `shape-outside` 属性是唯一相对比较成熟的，`shape-inside` 目前浏览器还不支持
...
用户可以体验到的最终结果，主要有以下几个方面

- **正确性：**代码是否如期运行？CSS 属性名都写对了吗？浏览器能否支持？
- **可用性：**代码运行后的结果不仅看起来没问题，而且还能使用？无障碍访问要解决这部分问题
- **健壮性：**万一出错以后怎么办？可以声明两套属性，其中一套用作旧版浏览器的后备
...
1. 浏览器首先会接收到网址对应的一个 HTML 文件，内容如下
...
2. 浏览器把这个 HTML 文件解析为一个对象(节点)树，这就是 DOM(文档对象模型 document object model)

   <img src="images/image-20240516091554891.png" alt="image-20240516091554891" style="zoom:50%;" />

3. 浏览器在碰到 HTML 中指向 CSS 文件的链接时，会获取 CSS 文件并解析为一个对象树(类似 DOM 树)，这就是 CSS 对象模型(CSSOM，CSS object model)，**浏览器会基于层叠、继承和特殊性来计算每个 DOM 节点的最终样式，DOM 和 CSSOM 都是标准化的，任何浏览器都应该相同，构建 CSSOM 需要依赖于 DOM**
   ...
   <img src="images/image-20240517061958441.png" alt="image-20240517061958441" style="zoom:50%;" />

4. 在构建完 DOM 和 CSSOM 之后，接下来是构建**渲染树**，在渲染树中，每个节点表示要渲染到屏幕上的信息，在渲染树中，被隐藏的 DOM 节点不会出现在这里，类似 `::before` 之类的伪元素则可能在这里有渲染对象，但在 DOM 里却没有节点，浏览器还会再这里保存其他表现性信息，比如滚动块或视口

   <img src="images/image-20240517063154994.png" alt="image-20240517063154994" style="zoom:50%;" />

   在构建完成的渲染树中，每个节点都应该知道自己是什么颜色，文本使用哪种字体显示以及是否有明确的宽高等等样式信息

   **注意：** 该渲染树是简化的渲染树，`head`、`title`、`meta` 等元素在这里没有渲染对象，`display: none;` 的元素也没有渲染对象

5. 接下里是布局，计算每个渲染对象的几何属性，这个过程叫**布局(layout)**或**重排(reflow)**，浏览器会遍历渲染树，确定每个对象在页面上的什么位置显示，有时候渲染属性的额外渲染对象需要在这个时候构建出来，例如文本在使用某种字体时会换行，原来的一个行盒子必须拆分为两个匿名行盒子，而增加行盒子势必会影响父元素高度以及周围其他元素

6. 最后就是绘制、合成与呈现，浏览器基于渲染树中的信息，把元素准确地呈现在屏幕上，在确定了每个渲染对象的位置和属性后，浏览器可以计算出它们在屏幕上占据的像素树，这个过程叫做**绘制**，如果发现图像的某一部分不影响页面其它内容的显示，也可能会拆分为不同的任务，每个任务负责绘制页面上**特定的部分(层)**，有些复杂的元素，例如经过三维变换的元素，可能需要通过 GPU 进行硬件加速，其他应用了滤镜或混合模式的元素，需要确定如何与其他层混合，需要先拆出层，再组合起来，这个过程叫做**合成**，最后把页面发给屏幕去呈现



**优化渲染性能**

如果页面上有了任何变化，浏览器就需要重复之前的步骤，为了保证屏幕上的页面能够平滑变化，最好能在 **16 毫秒**以内完成重绘，16 毫秒对应的屏幕刷新率为 60 HZ

有些变化不会影响性能，例如滚动页面，这时浏览器只要把整个渲染结果重新绘制到不同位置即可，如果某些变化会导致页面上的样式改变，就可能会影响性能

通过 Javascript 改变了某个元素的 `width` 和 `height`，浏览器就需要重新布局、合成和绘制，只改变文本颜色不需要重新布局，只会触发绘制及合成，总之，**最不会影响性能的操作，就是那些只能在合成阶段完成的操作**，可以通过 https://lab.skk.moe/css-triggers 知道哪个属性对应哪个渲染阶段的表格

还可以通过浏览器的开发者工具看到浏览器执行以上的步骤，在 Chrome 的开发者工具中的**性能**标签页中可以查看并记录交互操作是否会触发特定的渲染步骤

可以在 Chrome 的开发者工具的**渲染**标签页中勾选突出显示绘制区域，然后在与页面交互时，浏览器就会高亮显示重新绘制的区域

想要避免重新绘制的方法是强制浏览器为渲染区域创建一个独立的硬件加速层，然后只将它与页面其他部分合成起来，可以通过使用 `will-change` 属性，这个属性可以提示浏览器当前元素会在将来更新 `transform` 属性，单独一个 `transform` 属性并不会创建一个新层，但给变换加上动画会创建一个新层，这样当浏览器看到头部将来会以动画的形式出现提示时，就会从一开始就为它单独创建一个层
...
**注意：** `will-change` 应当视为最后的手段，浏览器已经尽力优化了所有东西，一些较强的优化可能与 `will-change` 关联，它们可能会使用大量的机器资源，过度使用会导致页面变慢或消耗大量资源



**内部代码质量：以人为本**

作为开发者，首先应该考虑用户的感受，而不是自己方不方便，因此要始终关注代码的外部质量，但也有人认为代码的内部的质量更重要，代码的内部质量需要从以下几个方面来衡量

- **复用性(DRY)：** (don't repeat yourself) 一个问题能否在一个地方解决，如果要修改方案，是否需要修改很多地方？
- **可读性：** 其他人阅读你的代码时，是否很快读懂？
- **可移植性：** 你写的代码是否可以独立使用，还是必须依赖项目的其它代码才能使用?
- **模块化：** 是否可以将代码以清晰的方式组织起来，放到其它地方重用?

外部代码质量高往往是内部质量高的结果，反之，如果内部质量不高，外部质量也很难高到哪去
...
1. 使用了 ID 选择符，ID 选择符无法复用，在这里完全没有必要，ID 属性主要是用于页内链接或者 JavaScript 的接入点，ID 属性存在高特殊性导致样式难以被覆盖
2. ID 选择符前还加了 `div`，目的是限定 ID 选择符，但是这个限定没有什么必要，还会增加特殊性
3. ID 属性的名字 `#pink-box`，如果后续想把消息框改成别的颜色时名字就对不上了
...
1. 将 ID 属性值修改为 class 值 `warning-message`
2. 文本、边框的颜色都通过半透明的黑色生成，再与粉色背景混合，意味着修改背景只需要修改一个地方，另外两个阴影都不用动
...
1. 此时 `.message`  规则为通用规则
2. `.message-warning` 规则在通用规则的基础上变化了背景颜色
...
OOCSS 表示 object oriendted CSS，面向对象的 CSS，把 CSS 当做可维护的软件来看待，类名(在语义正确的 HTML 标记基础上) 成为主要的载体

假设要做一个媒体对象，建议从结构中分离处皮肤、从内容中分离处容器

**从结构中分离处皮肤：** 不要把字体、颜色(即皮肤) 和定位、浮动(即结构)的样式写在一起，此时最好给这两类样式分别创建规则和类名
...
<article class="media-block post-teaser">
  ...
  <img src="" alt="" class="media-fig"  />
</article>
```

这里的 `post-类` 可以表示组件的皮肤，`media-类` 表示的组件的结构



**从内容中分离处容器：** 通过技术上使用 `.col` 元素来决定组件如何适应布局的样式，可以避免这些样式与组件本身的样式产生冲突
...
<div class="row row-trio">
    ...
    <article class="media-block post-teaser">
      ...
      <img src="" alt="" class="media-fig post-fig"  />
    </article>
    <!-- 其它 post-teaser -->
  </div>
</div>
...
SMACSS 表示 scalable and modular architecture for CSS(可伸缩及模块化的架构)，与 OOCSS 有很多地方类似，推崇以类名和专注组件的规则，主要提出以下规则分类法

- 基本样式，为 HTML 元素提供默认样式，同时基于元素提供不同的版本
- 布局样式，网格系统和其他布局的辅助，使用类似 `.col`、`.row`
- 模块样式，包含网站特定组件(产品、产品列表、网站头部)的所有样式，大部分样式都属于这一类
- 状态，用于改变已有模块的外观，如菜单有激活和未激活状态

在坚持以上这套分类法的同时，SMACSS 还提倡思考命名方式，要区分场景，正常情况下应该按照上述顺序包含在样式表中，从最通用到最具体，这样可以避免特殊性以及合理使用层叠

除了对样式进行分类，SMACSS 还提倡给类名加前缀，例如：`.l-` 前缀表示布局(layout)、`.is-` 前缀表示状态
...
/* 以下两种禁用状态都可以 */
.is-productlist-disabled {
  /* 产品列表禁用状态 */
}
.productlist-is-disabled {
  /* 产品列表禁用状态 */
}
```
...
BEM 包含一些约定、库和工具，用于在大规模 Web 应用中实现结构化 UI，BEM 是 block、element 和 modifier 的首字母缩写

- **block(块)：** 最高级抽象，对应于 SMACSS 中的模块或 OOCSS 中的对象，任何自成一体的东西都可以描述为块
- **element(元素)：** 表示块的子组件，并不是指 HTML 元素，它们不是相同的概念
- **modifier(修饰符)：** 表示块或元素的不同状态或变体

BEM 中的块、元素和修饰符全部使用小写，以连字符分隔
...
修饰符前面要加一个下划线，用于修饰块或元素

```css
.product-list_featured {
  /* 产品列表的变体 */
}
.product-list_featured__item {
  /* 产品列表变体中的项目元素 */
}
.product-list__item_sold-out {
  /* 常规产品列表中售罄的项目 */
}
```
...
更多的 BEM 建议参考 http://cssguidelin.es/

不管使用什么语法，BEM 的核心语法思想就是让某个类名相关规则能够一目了然，需要提前知道命名模式，这有助于保持代码专注，如果让一个类承担太多的责任，它就会与其它名字对应的用途相冲突，此时需要重新思考抽象的信号，可以按照自己的意图把复杂的块拆分为可嵌套的块，使这些块可以进一步重用



**管理复杂性**

> **关注点分离：** 标记中不应该包含表现性信息，也不应该依赖表现层，这两层之间的交集应该始终尽可能最小化

上述概念带入 Web 中是这样：就算没有 CSS(包括 Javascript)，网页本身也足以传达自身的信息

关注点分离的概念诞生于 Web 之前，主张要把关注点放在某个方面，而不是同时处理各个方面，但我们需要同事考虑 HTML、CSS 就已经违背了这条原则，但我们可以在代码的不同层面来使用关注点分离原则，使得可以在另一方面弥补理论

在 HTML 和 CSS 2.1 关于类名的描述中，都推荐使用语义化的标签和类名(类名应描述内容本质而非内容表现的名称)

无论怎么样命名类名和结构化规则，比不是二选一，任何元素都可以有描述内容的类名，也可以有描述它们在 UI 中功能的类名



**工具和流程**

因为 CSS 没有设计类似通用编程语言的那些语法，例如：循环、函数、列表、变量等，就有人为 CSS 设计了另一种语言，这就是**预处理器**

目前常见的预处理有：Sass(Scss)、Less、Stylus、PostCSS 等，目前 Sass(Scss) 是最流行的

Sass 与 Scss 都是 CSS 预处理器

- Sass 3.0 版本之前使用的扩展名是 `.sass`，语法上不要求使用 `{}` 和 `;`，不遵从 CSS 规范，不被大众所接受
...
- Sass 3.0 版本之后使用的扩展名是 `.scss`，和 CSS 写法无差别， 后来越来越受大众喜欢
...
Sass 是 CSS 的超集，SCSS 支持一切有效的 CSS 语法，此外加上了 Sass 特有的功能



**工作流工具**

很多代码编辑器都会内置语法检查功能，会高亮有可能错误的选择符或样式声明，一般称这种错误检查为 “静态分析”

如果要加强静态分析，可以使用工具 Linter，可以帮你找出代码中不应该存在的问题，对于 CSS 而言，有 `CSS Lint` 和 `Stylelint`，它们既可以检查语法错误，也能检查选择符或声明中可疑的模式



**构建工具**

在开发网站时，还有很多别的事情需要重复去做

- 预处理 CSS
- 拼接 CSS (如果有多个较短的 CSS 文件)
- 压缩 CSS，删除注释空白以节省空间
- 优化 CSS 中的图片
- 运行开发服务器
- 刷新浏览器以显示变化

以上步骤可以使用 Node.js 和 Gulp 等工具可以实现
...
- HTTP/2  与服务器推送(目前已支持)
  ...
  HTTP/2 底层协议的改进完成支持同时获取多个小型资源，一个连接可以传输多个文件，在新的协议下，之前的内联样式、精灵图等优化手段都可以退出历史舞台，`@import` 语句可以放心使用，不必担心请求过多

- Web 组件(目前已支持)

  Web 组件是指一组规范，可以让开发者把 HTML、CSS 和 Javascript 打包成一个真正完备、可复用的组件，就像是原生的元素一样，目前主流的新版本浏览器都已经支持，具体可以参考 https://developer.mozilla.org/zh-CN/docs/Web/API/Web_components

- CSS 与可扩展的 Web

  任何建议的新 CSS 特性都可以在构思阶段就以腻子脚本的形式实现出来
